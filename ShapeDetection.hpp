...
cv::Point ColorList[Cols] = {cv::Point(0,11), cv::Point(20,35), cv::Point(36,70), cv::Point(100,130)}; 
...
{
	cv::Mat temp = isolateColor(img, color.x, color.y, satMin, 255, 0, 255);
...
void getShapes(cv::Mat img, cv::Mat imgref, std::vector<cv::Point> &targets)
{
	...
	for(int i=0;i<contours.size();++i)
	{
		...
		if(shapeArea < ar)
			shapeArea = ar;
		if(ar >= 500)
			cv::drawContours(img,contours,i,cv::Scalar(0,0,255),1);	
	}
	std::cout << shapeArea << std::endl;
	cv::imshow("Test", img);
	cv::waitKey(0);
